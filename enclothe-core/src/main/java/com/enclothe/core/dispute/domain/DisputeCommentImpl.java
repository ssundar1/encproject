...
@Table(name = "ENC_DISPUTE_COMMENTS")
@Cache(usage=CacheConcurrencyStrategy.NONSTRICT_READ_WRITE, region="blStandardElements")
//@SQLDelete(sql="UPDATE ENC_DISPUTE_COMMENTS SET END_DATE = now() WHERE DISPUTE_COMMENTS_ID = ?")
public class DisputeCommentImpl implements DisputeComment {

	private static final long serialVersionUID = 1L;
	
	@Id
    @GeneratedValue(generator= "DisputeCommentId")
    @GenericGenerator(
        name="DisputeCommentId",
        strategy="org.broadleafcommerce.common.persistence.IdOverrideTableGenerator",
        parameters = {
            @Parameter(name="segment_value", value="DisputeCommentImpl"),
            @Parameter(name="entity_name", value="com.enclothe.core.dispute.domain.DisputeCommentImpl")
        }
    )
    @Column(name = "DISPUTE_COMMENTS_ID", nullable=false)
    protected Long id;
	...
	@OneToOne(targetEntity = DisputeChannelImpl.class)
